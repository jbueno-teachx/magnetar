...
from magnetar.particles import ElectroParticleSprite, ParticleSprite
...
def test_particle_color_at_construction() -> None:
    p = ElectroParticleSprite(meters(0, 0, 0), charge=coulomb(-1), color="light_blue")
    assert p.color == "light_blue"
    assert p.charge == coulomb(-1)
    n = ParticleSprite(meters(0, 0, 0), color="green", mass=gram(1))
